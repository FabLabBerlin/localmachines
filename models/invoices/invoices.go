...
func Create(locationId int64, interval Interval) (*Invoice, error) {
...
func CalculateSummary(locationId int64, interval Interval) (invoice Invoice, err error) {
...
func getPurchases(locationId int64, interval Interval) (ps []*purchases.Purchase, err error) {
	...
	query := fmt.Sprintf("SELECT p.* FROM %s p JOIN %s u ON p.user_id=u.id "+
		"WHERE MONTH(p.time_start) = ? AND YEAR(p.time_start) = ? "+
		"AND (p.running IS NULL OR p.running = 0) AND location_id = ?",
		p.TableName(),
		usr.TableName())

	_, err = o.Raw(query, interval.MonthFrom, interval.YearFrom, locationId).
		QueryRows(&ps)

	return
}

func (this *Invoice) getFileName(interval Interval) string {
...
func (this *Invoice) getPurchases(locationId int64, interval Interval) (ps []*purchases.Purchase, err error) {
