...
import (
	"errors"
	"fmt"
	r "github.com/FabLabBerlin/localmachines/lib/redis"
...
func init() {
	...
	if runmode == "dev" || runmode == "prod" {
		...
		store, err = redistore.NewRediStoreWithPool(r.GetPool(), secret)
		if err != nil {
			panic(err.Error())
		}
...
func (this *Controller) Prepare() {
	...
	switch path {
	case "/api/users/current", "/api/users/forgot_password", "/api/users/forgot_password/phone", "/api/users/forgot_password/reset", "/api/machine_types", "/api/machines/search", "/api/locations", "/api/locations/my_ip", "/api/metrics/realtime", "/api/reservations/icalendar", "/api/settings/terms_url", "/api/newsletters/easylab_dev", "/api/locations/jabber_connect", "/api/locations/debug", "/api/users/oauth2/login":
	default:
